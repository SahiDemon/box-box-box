...
for r in races:
    ...
    feats = {}
    for pos_key, p in r["strategies"].items():
        strat = p
        pit_map = {}
        ...
        d_arr = np.zeros(20)
        # multiply by base_lap_time * total_laps so it's a percentage of lap time!
        d_arr[drivers.index(p["driver_id"])] = base * float(r["race_config"]["total_laps"])
        
        # let's also add start position times base_lap_time (grid penalty may be proportional to track time or fixed)
        start_pos = int(pos_key[3:])
        
        feats[p["driver_id"]] = np.concatenate([np.array(x), d_arr, [start_pos, start_pos * pit_lane_time, start_pos * base]])
