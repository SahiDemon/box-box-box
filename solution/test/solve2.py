...
X_diff, Y = [], []
np.random.seed(42)
for file in glob.glob("data/historical_races/races_0*.json")[:2]: 
    with open(file) as f:
        data = json.load(f)
    for r in data:
...
lr = LogisticRegression(fit_intercept=False, C=10.0, max_iter=2000, solver='lbfgs')
lr.fit(X_diff, Y)
print(f"Accuracy: {lr.score(X_diff, Y)*100:.2f}%")
...
# Now save these to model_params.json
params = {
