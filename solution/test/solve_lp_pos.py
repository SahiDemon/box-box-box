...
for r in races:
    ...
    for pos_key, p in r["strategies"].items():
        ...
        # we append start_pos as a single linear feature
        # we can also add grid penalty: e.g. start_pos * 1.0
        feats[p["driver_id"]] = np.concatenate([np.array(x), d_arr, [start_pos]])
